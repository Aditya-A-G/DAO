...
export default function Home(){
  ...
  const fetchAllProposals = async ()=>{
    console.log("fetchAllProposals function called")
    try {
    console.log("fetchAllProposals function called")
      const proposalsArr = [];
      for(let i = 0; i< numProposals; i++){
        const proposalN = await fetchProposalById(i);
        proposalsArr.push(proposalN);
        console.log(proposalN)
      }
    ...
    if(loading){
      return (
        <div className={styles.description}>
          Loading... Waiting for transcation...
        </div>
      );
            ...
            <div key={index} className={styles.proposalCard}>
              ...
              {p.deadline.getTime() > Date.now() && !p.executed ? (
                <div className={styles.flex}>
                  <button className={styles.button} onClick={()=>voteOnProposal(p.proposalId, "YAY")}>
                    Vote YAY
                  </button>
                  <button className={styles.button} onClick={()=> voteOnProposal(p.proposalId, "NAY")}>
                    Vote NAY
                  </button>
                </div>
              ): p.deadline.getTime() < Date.now() && !p.executed ?(
                <div className={styles.flex}>
                  <button className={styles.button} onClick={()=>executeProposal(p.proposalId)}>
                    Execute Proposal{" "}
